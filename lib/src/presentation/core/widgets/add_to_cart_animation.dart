...
/// No-ops if [cartKey] isn't mounted, or is still hidden as of the next
/// frame (e.g. the target FAB was `SizedBox.shrink()`-empty and this add
/// didn't cause it to appear for some other reason).
///
/// Measurement is deferred to a post-frame callback: [cartKey]'s widget
/// (e.g. a cart FAB that only appears once the cart is non-empty) may not
/// have rebuilt with its real, visible size yet at the moment this is
/// called — state changes made just before calling this haven't been laid
/// out yet. Measuring immediately would read last frame's stale geometry
/// (a zero-size box on the very first add of a session), animating to a
/// vanishing point instead of the now-visible button.
void runAddToCartAnimation({
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    if (cartRenderObject.size == Size.zero) return;
